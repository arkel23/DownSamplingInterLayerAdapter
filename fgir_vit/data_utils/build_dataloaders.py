...
from .build_transform import build_transform, build_pipeline
...
def get_loader(args, split):
    if args.ffcv:
        _ = get_set(args, split)

        pipelines = build_pipeline(args, split)
        ...
        if args.dataset_name in ['imagenet', 'inat17']:
            os_cache = False
            if args.distributed:
                order = OrderOption.SEQUENTIAL
        else:
        ...
        distributed = True if args.distributed else False

        if split == 'train':
            fn = args.df_train.replace('.csv', '.beton')
        elif split == 'val':
            fn = args.df_val.replace('.csv', '.beton')
        elif split == 'test':
            fn = args.df_test.replace('.csv', '.beton')

        beton_path = os.path.join(args.dataset_root_path, fn)
        data_loader = Loader(
            beton_path, batch_size=args.batch_size, num_workers=args.cpu_workers,
            drop_last=drop_last, order=order, pipelines=pipelines,
            os_cache=os_cache, distributed=distributed, seed=args.seed)
