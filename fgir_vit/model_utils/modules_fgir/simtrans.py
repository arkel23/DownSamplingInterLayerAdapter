...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        config.seq_len_post_reducer = n_patches + 1
...
class SIMTrans(nn.Module):
    # def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0):
    def __init__(self, model_name='vit_b16', img_size=224, num_classes=21843):
        super(SIMTrans, self).__init__()
        self.cfg = CONFIGS[model_name]
        self.num_classes = num_classes
        # self.smoothing_value = smoothing_value
        self.zero_head = self.cfg.zero_head
        self.classifier = self.cfg.classifier
        self.transformer = Transformer(self.cfg, img_size)

        self.part_head = nn.Sequential(
            nn.BatchNorm1d(self.cfg.hidden_size*3),
            Linear(self.cfg.hidden_size*3, 1024),
            nn.BatchNorm1d(1024),
            nn.ELU(inplace=True),
            Linear(1024, num_classes),
        )

    def forward(self, x, labels=None, ret_dist=None):
    # def forward(self, x):
